...
<center><img src="/images/posts/2018-02-05-generative-modeling-and-ai/learning_process.gif" width=520 alt="Пример работы системы"/></center>
...
Положим $P(x)=N(x;\left(\begin{matrix}5 \\ 10\end{matrix}\right), \left(\begin{matrix} 1.2^2 & 0 \\ 0 & 2.4^2\end{matrix}\right))$ &mdash; двумерное нормальное распределение с диагональной ковариационной матрицей. $P(z)=N(z; 0, 1)$ &mdash; одномерное нормальное латентное представление.
...
$W$, в свою очередь, сошлось к примерно такому значению, при котором $W W^T + \sigma^2 I=\\begin{pmatrix} 1.2^2 & 0 \\\\ 0 & 2.4^2 \\end{pmatrix}$. Подставляя эти значения в модель, мы получаем $P(x)=N(x; b, W W^T + \sigma^2 I)=N(x; \begin{pmatrix} 5 \\\\ 10 \end{pmatrix}, \begin{pmatrix} 1.2^2 & 0 \\
\\ 0 & 2.4^2 \end{pmatrix})$, что значит, что мы восстановили распределение данных.
