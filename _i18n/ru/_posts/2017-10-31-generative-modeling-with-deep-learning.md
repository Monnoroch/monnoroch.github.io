...
Давайте посмотрим, как это работает на простом примере. Положим $X=R$, $Y=\\{0, 1\\}$, $p(x|y=0)=N(x; \mu\_1,\sigma\_1)$, $p(x|y=1)=N(x; \mu\_2,\sigma\_2)$, $p(y=0)=p(y=1)=1/2$. То есть, у нас есть две гауссианы, из которых мы равновероятно сэмплируем данные и нам нужно, имея точку из $R$, предсказать, из какой гауссианы она была получена.
...
Т.е.
\\begin{equation}
    p(y=i|x)=\\frac{N(x;\mu\_i, \sigma\_i)}{\\displaystyle\sum_{l=1,2}{N(x;\mu\_l, \sigma\_l)}}
\\end{equation}
